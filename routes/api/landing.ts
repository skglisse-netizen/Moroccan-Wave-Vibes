...
router.post("/spots", authenticate, checkPermission('add_landing_page'), async (req: AuthRequest, res: Response) => {
  const { name, description, lat, lng, image_url, difficulty, suggestion_type, suggestion_name, suggestion_link } = req.body;
  try {
    const result = await query("INSERT INTO spots (name, description, lat, lng, image_url, difficulty, suggestion_type, suggestion_name, suggestion_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [name, description, lat, lng, image_url, difficulty, suggestion_type, suggestion_name, suggestion_link]);
    res.json({ id: result.lastInsertRowid });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.put("/spots/:id", authenticate, checkPermission('change_landing_page'), async (req: AuthRequest, res: Response) => {
  const { name, description, lat, lng, image_url, difficulty, suggestion_type, suggestion_name, suggestion_link } = req.body;
  try {
    await query("UPDATE spots SET name = ?, description = ?, lat = ?, lng = ?, image_url = ?, difficulty = ?, suggestion_type = ?, suggestion_name = ?, suggestion_link = ? WHERE id = ?",
      [name, description, lat, lng, image_url, difficulty, suggestion_type, suggestion_name, suggestion_link, req.params.id]);
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post("/landing-page", authenticate, checkPermission('change_landing_page'), async (req, res) => {
  const { 
    ...
    show_logo, show_button, button_link, section_button_label
  } = req.body;
  try {
    await query(`
...
router.get("/public-services", authenticate, checkPermission('view_landing_page'), async (_req, res) => {
