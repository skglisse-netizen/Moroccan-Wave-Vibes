  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ full_name: '', phone: '', email: '', address: '', is_subscriber: false, total_sessions: 0, remaining_sessions: 0, matricule: '' });
      }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        {hasPermission(user, 'add_clients') && (
          <button
            onClick={() => { setEditingItem(null); setSelectedServiceId(''); setFormData({ full_name: '', phone: '', email: '', address: '', is_subscriber: false, total_sessions: 0, remaining_sessions: 0, matricule: '' }); setShowModal(true); }}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                <th className="px-4 py-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-left">
                ...
                <th className="px-4 py-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">Matricule</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginatedData.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50/50 transition-all">
                  ...
                  <td className="px-4 py-2 text-center text-[10px] font-mono font-bold text-slate-600">
                    {c.matricule || '-'}
                  </td>
        ...
        <Modal title={editingItem ? "Modifier le Client" : "Nouveau Client"} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input 
                label="Matricule" 
                value={editingItem ? formData.matricule : "Auto-généré (CL0000)"} 
                readOnly 
