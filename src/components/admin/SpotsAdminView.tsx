    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => { setEditingItem(null); setFormData({ name: '', description: '', lat: '', lng: '', difficulty: 'Débutant', image_url: '', suggestion_type: '', suggestion_name: '', suggestion_link: '' }); setShowModal(true); }}>
                ...
                <tr key={s.id} className="hover:bg-slate-50/50 transition-all group">
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button
                        onClick={() => { setEditingItem(s); setFormData({ name: s.name, description: s.description, lat: s.lat.toString(), lng: s.lng.toString(), difficulty: s.difficulty, image_url: s.image_url || '', suggestion_type: s.suggestion_type || '', suggestion_name: s.suggestion_name || '', suggestion_link: s.suggestion_link || '' }); setShowModal(true); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-100 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <Input label="Nom de l'établissement" value={formData.suggestion_name} onChange={(e) => setFormData({ ...formData, suggestion_name: e.target.value })} />
              </div>
              <Input label="Lien (URL Google Maps ou Site Web)" value={formData.suggestion_link} onChange={(e) => setFormData({ ...formData, suggestion_link: e.target.value })} />
            </div>
