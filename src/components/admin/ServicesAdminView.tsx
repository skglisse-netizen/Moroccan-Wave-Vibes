              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <Select
                    ...
                    value={configForm.services_layout}
                    onChange={(e) => setConfigForm({ ...configForm, services_layout: e.target.value })}
                    options={[
                      { value: 'single-row-centered', label: 'Ligne centrée (Sans défilement)' },
                      { value: 'grid-4', label: 'Grille (4 colonnes)' },
                      { value: 'grid-5', label: 'Grille (5 colonnes)' },
            ...
            <div className="flex items-center gap-2">
              {s.discount_percentage > 0 ? (
                <div className="flex flex-col">
                  <span className="text-[10px] text-black line-through font-bold">{s.price} DH</span>
        ...
        <Modal title={editingService ? "Modifier le service" : "Nouveau service"} onClose={() => setShowServiceModal(false)} maxWidth="max-w-lg">
          <form onSubmit={handleServiceSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Image / Aperçu</label>
              <div className="flex gap-2 items-center">
                <Input 
                  className="flex-1"
                  placeholder="URL de l'image" 
                  value={serviceForm.image_url} 
                  onChange={(e: any) => setServiceForm({ ...serviceForm, image_url: e.target.value })} 
                />
                <div className="relative shrink-0">
                  <input type="file" accept="image/*" onChange={handleImageUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                  <Button type="button" variant="secondary"><Upload size={16} /> Parcourir</Button>
                </div>
              </div>
