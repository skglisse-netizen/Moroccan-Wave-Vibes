...
export function AboutAdminView({ content, onUpdate, user, onUpdateContent }: { 
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'video') => {
    ...
    if (data) {
      setAboutForm(prev => ({ ...prev, [type === 'image' ? 'image_url' : 'video_url']: data.url }));
    }
  };
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {showConfig && (
          ...
          >
            <Card className="p-6 border-indigo-100 bg-indigo-50/10 mb-6 space-y-6">
              
              {/* Configuration Générale */}
              <div className="flex flex-col md:flex-row items-end gap-6 pb-6 border-b border-indigo-100/50">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col md:flex-row items-end gap-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                    <div className="flex-1 w-full">
                      <Input
                    ...
                    <div className="flex-1 w-full">
                      <Input
                        label="Lien (ex: reserve, spots, contact)"
                    ...
                    <div className="flex flex-col items-center justify-center shrink-0 md:px-4">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 mb-2 text-center">Activer</label>
                      <button
                        type="button"
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col md:flex-row items-end gap-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm opacity-90">
                    <div className="flex-1 w-full">
                      <Input
                        label="Texte du bouton"
                        defaultValue={sectionData.button_label_2}
                        onBlur={(e: React.FocusEvent<HTMLInputElement>) => {
                          const newVal = e.target.value;
                          if (newVal !== sectionData.button_label_2) {
                            onUpdateContent('about', { ...sectionData, button_label_2: newVal });
                          }
                        }}
                    ...
                    <div className="flex-1 w-full">
                      <Input
                        label="Lien (ex: spots, contact)"
                        defaultValue={sectionData.button_link_2}
                        onBlur={(e: React.FocusEvent<HTMLInputElement>) => {
                          const newVal = e.target.value;
                          if (newVal !== sectionData.button_link_2) {
                            onUpdateContent('about', { ...sectionData, button_link_2: newVal });
                          }
                        }}
                      />
                    </div>
                    <div className="flex flex-col items-center justify-center shrink-0 md:px-4">
                      ...
                      <button
                        type="button"
                        onClick={() => onUpdateContent('about', { ...sectionData, show_button_2: !Boolean(sectionData.show_button_2 ?? true) })}
                        className={`w-12 h-6 rounded-full relative transition-colors ${Boolean(sectionData.show_button_2 ?? true) ? 'bg-indigo-500' : 'bg-slate-200'}`}
                      >
