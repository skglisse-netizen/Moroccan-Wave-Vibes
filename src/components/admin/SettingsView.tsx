    ...
    <Card className="max-w-4xl mx-auto p-8">
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="pt-6 border-t border-slate-100 space-y-6">
          <h3 className="text-lg font-bold text-slate-800">📧 Email (SMTP) — Envoi depuis la page Contact</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input label="Serveur SMTP (ex: smtp.gmail.com)" value={formData.smtp_host} onChange={(e) => setFormData({ ...formData, smtp_host: e.target.value })} placeholder="smtp.gmail.com" />
            <Input label="Port SMTP (ex: 587)" value={formData.smtp_port} onChange={(e) => setFormData({ ...formData, smtp_port: e.target.value })} placeholder="587" />
            <Input label="Utilisateur SMTP (email)" value={formData.smtp_user} onChange={(e) => setFormData({ ...formData, smtp_user: e.target.value })} placeholder="votre@email.com" />
            <Input label="Mot de passe SMTP" type="password" value={formData.smtp_pass} onChange={(e) => setFormData({ ...formData, smtp_pass: e.target.value })} placeholder="••••••••" />
            <Input label="Email expéditeur (From)" value={formData.smtp_from} onChange={(e) => setFormData({ ...formData, smtp_from: e.target.value })} placeholder="noreply@votre-ecole.com" />
            <Input label="Email destinataire (To — reçoit les messages)" value={formData.smtp_to} onChange={(e) => setFormData({ ...formData, smtp_to: e.target.value })} placeholder="contact@votre-ecole.com" />
          </div>
          <p className="text-xs text-slate-400 font-medium">
            💡 Gmail : activez "Mots de passe d'application" dans votre compte Google et utilisez ce mot de passe ici. Port 587 (TLS) ou 465 (SSL).
          </p>
        </div>
